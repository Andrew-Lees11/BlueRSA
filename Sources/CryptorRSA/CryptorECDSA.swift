...
//        // MacOS, iOS ect.
//
//        // Hash using common crypto sha256
...
//
//
//    static func signAndVerify(data: Data, publicKey: PublicKey, privateKey: PrivateKey) -> Bool {
////        let dataBytes = [UInt8](data)
////        let signature = ECDSA_do_sign(dataBytes, Int32(dataBytes.count), privateKey.nativeKey)
////        let result = ECDSA_do_verify([UInt8](data), Int32(data.count), signature, publicKey.nativeKey)
//
...
//            signedBytesLength.deallocate()
//        }
//        ECDSA_sign(0, dataBytes, Int32(dataBytes.count), signedBytes, signedBytesLength, eckey)
////        let signature = ECDSA_do_sign(dataBytes, Int32(dataBytes.count), eckey)
////
////        let r = signature?.pointee.r
////        let rHexCString = BN_bn2hex(r)!
////        let rString = String(cString: rHexCString)
////        let s = signature?.pointee.s
////        let sHexCString = BN_bn2hex(s)!
////        let sString = String(cString: sHexCString)
////        let signatureData = CryptorECDSA.ASN1FromSignature(rHex: rString, sHex: sString)
////        let signatureBytes = [UInt8](signatureData)
//        let verify = ECDSA_verify(0, dataBytes, Int32(dataBytes.count), signedBytes, Int32(signedBytesLength.pointee), eckey)
//        // ECDSA_do_verify takes the signature object however we will be storing it as data
//        //let result = ECDSA_do_verify([UInt8](data), Int32(data.count), signature, eckey)
//        return (verify == 1)
//    }
//
//    public static func createSignature(data: Data, privateKey: PrivateKey) -> Data? {
...
//        //#if os(Linux)
//
//        // Need to sha databytes before signing
//        // SHA256 Digest must be exactly 32 bytes(CC_SHA256_DIGEST_LENGTH) for asymmetric encryption.
//        var hash = [UInt8](repeating: 0, count: Int(CC_SHA256_DIGEST_LENGTH))
//        CC_SHA256((data as NSData).bytes, UInt32(data.count), &hash)
//        let BytesData = Data(bytes: hash)
//        let dataBytes = [UInt8](BytesData)
//
//        let signedBytes = UnsafeMutablePointer<UInt8>.allocate(capacity: 73)
//        let signedBytesLength = UnsafeMutablePointer<UInt32>.allocate(capacity: 1)
//        defer {
...
//    }
//
////    static func createSignature(data: Data, privateKey: PrivateKey) -> Data? {
////
////// MacOS
//////        var hash = [UInt8](repeating: 0, count: Int(CC_SHA256_DIGEST_LENGTH))
//////        CC_SHA256((data as NSData).bytes, CC_LONG(data.count), &hash)
...
////
//////        let signature = ECDSA_do_sign(hash, Int32(hash.count), privateKey.nativeKey)
//////        let r = signature?.pointee.r
//////        let rHexCString = BN_bn2hex(r)!
//////        let rString = String(cString: rHexCString)
//////        let s = signature?.pointee.s
...
//////        let sString = String(cString: sHexCString)
//////        let signatureData = CryptorECDSA.ASN1FromSignature(rHex: rString, sHex: sString)
//////        print(signatureData.base64EncodedString())
////    }
//
//    // Verify the signature using the given public key.
//    public static func verifySignature(digestData: Data, signatureData: Data, publicKey: PublicKey) -> Bool {
...
//    }
//
////    static func verifySignature(digestData: Data, signatureData: Data, publicKey: PublicKey) -> Bool {
//////        var hash = [UInt8](repeating: 0, count: Int(CC_SHA256_DIGEST_LENGTH))
//////        CC_SHA256((digestData as NSData).bytes, CC_LONG(digestData.count), &hash)
//////        let digestData = Data(bytes: hash)
//////
...
////
//////        var hash = [UInt8](repeating: 0, count: Int(CC_SHA256_DIGEST_LENGTH))
//////        CC_SHA256((digestData as NSData).bytes, UInt32(digestData.count), &hash)
//////        let digestData = Data(bytes: hash)
//////        let signatureBytes = [UInt8](signatureData)
//////        let verify = ECDSA_verify(0, hash, Int32(hash.count), signatureBytes, Int32(signatureBytes.count), publicKey.nativeKey)
//////        print(verify)
//////        let (result, _) = toASN1Element(data: signatureData)
//////        guard case let ASN1Element.seq(elements: es) = result,
//////            es.count > 1,
//////            case let ASN1Element.bytes(data: rData) = es[0],
//////            case let ASN1Element.bytes(data: sData) = es[1]
//////        else {
//////                return false
//////        }
//////        let rBytes: [Int8] = rData.map{Int8(bitPattern: $0)}
//////        let rBigNum = BN_new()
//////        let rBigNumPointer: UnsafeMutablePointer<UnsafeMutablePointer<BIGNUM>?>? = nil
//////        rBigNumPointer?.initialize(to: rBigNum)
//////        BN_hex2bn(rBigNumPointer, rBytes)
//////
//////        let sBytes: [Int8] = sData.map{Int8(bitPattern: $0)}
//////        let sBigNum = BN_new()
//////        let sBigNumPointer: UnsafeMutablePointer<UnsafeMutablePointer<BIGNUM>?>? = nil
//////        sBigNumPointer?.initialize(to: sBigNum)
//////        BN_hex2bn(sBigNumPointer, sBytes)
//////        let ecdsaSig = ECDSA_SIG_st(r: rBigNumPointer?.pointee, s: sBigNumPointer?.pointee)
//////        let ecdsaSigPointer: UnsafeMutablePointer<ECDSA_SIG>? = nil
//////        ecdsaSigPointer?.initialize(to: ecdsaSig)
//////        let verifyResult = ECDSA_do_verify([UInt8](digestData), Int32(digestData.count), ecdsaSigPointer, publicKey.nativeKey)
//////
...
////
////            let keyData = publicKeyData.drop(while: { $0 == 0x00}) + privateKeyData
//            guard let nativeKey = CryptorECDSA.PrivateKey.keyDataToNativeKey(key: pemKey.data(using: .utf8)!) else {
//                return nil
//            }
//            self.nativeKey = nativeKey
//        }
...
//    }
//
//    private static func dataWithHexString(hex: String) -> [UInt8] {
//        var hex = hex
//        var data = [UInt8]()
//        while(hex.count > 0) {
...
//    }
//
//    private static func ASN1FromSignature(rHex: String, sHex: String) -> Data {
//        let rlength = String(rHex.count/2, radix: 16)
//        let slength = String(sHex.count/2, radix: 16)
//        let totalLength = rHex.count/2 + sHex.count/2 + 4
//        let totalLengthHex = String(totalLength, radix: 16)
//        var asnData = Data(repeating: 0x00, count: totalLength + 2)
//        asnData[0] = 0x30
//        asnData[1] = dataWithHexString(hex: totalLengthHex)[0]
//        asnData[2] = 0x02
//        asnData[3] = dataWithHexString(hex: rlength)[0]
//        let rData = dataWithHexString(hex: rHex)
//        for i in 0 ..< rData.count {
//            asnData[i + 4] = rData[i]
//        }
//        asnData[4 + rData.count] = 0x02
//        asnData[5 + rData.count] = dataWithHexString(hex: slength)[0]
//        let sData = dataWithHexString(hex: sHex)
//        for i in 0 ..< sData.count {
//            asnData[i + 6 + rData.count] = sData[i]
//        }
//        return asnData
//    }
//}
